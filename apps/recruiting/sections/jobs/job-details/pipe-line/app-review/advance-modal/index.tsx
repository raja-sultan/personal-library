...
export function AdvanceModal({ open, setOpen, candidateData }): JSX.Element {
  const index = candidateData?.details?.findIndex(
    (i) => i.stage === candidateData?.stage
  );
  const currentStage = candidateData?.details?.[index];
  const NextStage = candidateData?.details?.[index + 1];
  const rejectedData = candidateData?.details?.find(
    (i) => i.stage === "rejected"
  );
  ...
  return (
    <CustomModal
      ...
      headerLabel={`${candidateData?.stage} for ${candidateData?.job}`}
      closeButtonProps={{
        onClick: () => {
          setOpen(false);
        },
