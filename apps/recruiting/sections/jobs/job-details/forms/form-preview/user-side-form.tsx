...
import { useGetFormByIdQuery } from "@services/jobs/job-details/forms/forms-api";
...
export default function UserSideForm(): JSX.Element {
  const searchParams = useSearchParams();
  const formId = searchParams.get("formId");
  const candidateId = searchParams.get("candidateID");

  const { data: defaultData } = useGetFormByIdQuery({ id: formId });
  let allQuestions: any = [];

  defaultData?.sections?.forEach((element) => {
  ...
  const onSubmit = (data): void => {
    console.log({ formData: data, formId, candidateId });
  };
  return (
    <Box
      display="flex"
    ...
    >
      ...
      <Box width="100%" boxShadow={4} p={2} borderRadius={2}>
        <Typography variant="h4">{defaultData?.form?.formName}</Typography>
        <Typography variant="body1">
          Stage: {defaultData?.form?.jobStage}
        </Typography>
        ...
        <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
          ...
          <Box display="flex" justifyContent="flex-end" mt={2}>
            {Boolean(defaultData?.sections?.length) && (
