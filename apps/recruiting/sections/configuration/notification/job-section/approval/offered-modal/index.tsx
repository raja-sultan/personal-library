...
export function OfferedModal({ isOpen, closeModel, jobId }: any): JSX.Element {
  ...
  const onSubmit = handleSubmit(async (data: any) => {
    ...
    try {
      const res: any = await updateNotifications({
        body: {
          notificationFor: "offerFullyApproved",
          users,
        },
        jobId,
      }).unwrap();
  ...
  return (
    <CustomModal
      ...
      onClose={closeModel}
      headerLabel="Offer Fully Approved"
        ...
        <Grid container>
          <Grid item xs={12} mt={1}>
            <Typography variant="body2">
              For each Candidate offer is fully approved, emails will be sent
              to:
            </Typography>
