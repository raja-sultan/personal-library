...
import { useDeleteCandidateTagMutation } from "@services/configuration/candidate-tags-api/candidate-tags-api";

export default function DeleteCandidateTagModalBox(props): JSX.Element {
  const { openDeleteModal, setOpenDeleteModal, idForUpdateDelete } = props;
  const theme = useTheme();
  const [deleteCandidateTag, { isLoading }] = useDeleteCandidateTagMutation();

  const handleDelete = async (): Promise<void> => {
    try {
      await deleteCandidateTag(idForUpdateDelete);
      toast.success(`Candidate Tag deleted successfully!`);
