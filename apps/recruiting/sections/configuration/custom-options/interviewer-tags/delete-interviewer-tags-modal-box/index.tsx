...
import { useDeleteInterviewerTagsMutation } from "@services/configuration/interviewer-tags-api/interviewer-tags-api";
import { CustomModal } from "common";
import React from "react";
import toast from "react-hot-toast";
import HighlightOffIcon from "@mui/icons-material/HighlightOff";

export default function DeleteInterviewerTagModalBox(props): JSX.Element {
  const { openDeleteModal, setOpenDeleteModal, idForUpdateDelete } = props;
  const theme = useTheme();
  const [deleteTag, { isLoading }] = useDeleteInterviewerTagsMutation();

  const handleDelete = async (): Promise<void> => {
    try {
      await deleteTag(idForUpdateDelete);
      toast.success(`Interviewer Tags deleted successfully!`);
