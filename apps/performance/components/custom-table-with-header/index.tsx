...
export function CustomTableWithHeader(
  props: CustomTableWithHeaderProps
): JSX.Element {
  const {
    primaryHeader,
    primaryHeaderProps,
  ...
  return (
    <Box sx={styles.tableWrapper}>
      {(primaryHeader || secondaryHeader) && (
        <Card
          sx={styles.tableHeaderWrapper(Boolean(children))}
          classes={{ root: "_root" }}
        >
          {primaryHeader && (
            <TablePrimaryHeader
              {...primaryHeaderProps}
              removeMargin={secondaryHeader}
            />
          )}
          {secondaryHeader && (
            <TableSecondaryHeader {...secondaryHeaderProps} />
          )}
